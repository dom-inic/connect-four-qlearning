# to play my version of connect3 use the left and right arrow keys to move the coins back and forth
# use the enter button to drop the coin to the bottom of the screen. 
# to play with the agent select the play vs agent options. the agent is trained so you will have to be tough to beat it
#  you can train the agent from scratch by selecting the train agent option
# you can also play with your friend by selecting the 2 player mode 
#  the agent has been trained using the q learning approach 
...
# class button for displaying game buttons
class Button():
    """ buttons class"""
    def __init__(self, screen, msg, posx, posy):
        ...
        def draw_button(self):
            """ draw button method that can be called to display the button"""
...
class Board():
    """A class to represent the connect 3 board"""
    ...
    def __init__(self, num_rows, num_columns):
        """
        Initialize a board with num_rows rows and num_columns 
        """
...
class GameView(object):
    ...
    def draw_menu(self):
        ...
        self.title_surface = font.render('connect 3', True, BLACK)
        fw, fh = font.size('connect 3')
        self.background.blit(self.title_surface, ((self.width - fw) // 2, 150))
        computer_player_text = 'play vs Agent'
        train_text = 'train Agent'
        two_player_text = '2 Player '
        ...
        # restart_button = Button(  "Restart", 800, 0)
...
reward = [
    1, 0.5 , -2, 1, -2, 1, 0.5, 0.5, 1, 1,
    0.5, 0.5 , 1, -2, 1, -2, 0.5, 0.5, 1, -2,
    1, 0.5 , 1, 1, -2, 1, 0.5, 0.5, 1, 1,
    -2, 0.5 , 0.5, 0.5, -2, .2, 0.5, 0.5, 1, -2,
    -2, 0.5 , 1, 1, -2, 1, 0.5, 0.5, -2, -2]


class Coin():
    """A class that represents the coin pieces used in connect 3"""
...
if __name__ == "__main__":
    ...
    parser.add_argument('iterations', nargs='?', default=50, action="store", help="Store the number of iterations to train computer")
    args = parser.parse_args()

    GameView(1000, 500).main_menu(int(args.iterations))
